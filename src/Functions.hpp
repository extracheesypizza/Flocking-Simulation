...
static vector<float> random2DVector(float lo, float hi)
{
    // returns a random 2D vector w/ values from lo to hi
    return {randomFloat(lo, hi), randomFloat(lo, hi)};
}

static void limit(float& variable, float value)
{
    if(variable > value)
        variable = value;
...
static void setMagnitude(vector<float>& v, float newMagnitude)
{
